...
def generateProposals(set_name,video_name,actionLabel):
    ...
    startend_scores=tdf.startend.values[:]
    max_startend = max(startend_scores)
    
    startend_bins=numpy.zeros(len(startend_scores))
    startend_bins[[0,-1]]=1
    for idx in range(1,tscale-1):
        if startend_scores[idx]>startend_scores[idx+1] and startend_scores[idx]>startend_scores[idx-1]:
            startend_bins[idx]=1
        elif startend_scores[idx]>(peak_thres*max_startend):
            startend_bins[idx]=1
    ...
    for j in range(tscale):
        if startend_bins[j]==1:
            xmin_list.append(tgap/2+tgap*j)
            xmin_score_list.append(startend_scores[j])
        if startend_bins[j]==1:
            xmax_list.append(tgap/2+tgap*j)
            xmax_score_list.append(startend_scores[j])
