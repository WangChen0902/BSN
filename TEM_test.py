...
def TEM_inference(X_feature,config):
    ...
    scores={"anchors_action":anchors_action,
            "anchors_startend":anchors_startend}
    return scores
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            batch_result_startend.append(out_scores["anchors_startend"])
    ...
    columns=["action","startend","xmin","xmax"]
    print(len(out_scores["anchors_action"]))
    print('action:',len(batch_result_action))
    print('startend:',len(batch_result_startend))
    ...
    for idx in range(list_len):
        ...
        b_startend=batch_result_startend[idx]
        ...
        for j in range(len(b_action)):
            ...
            tmp_startend=b_startend[j]
            tmp_xmin=b_xmin[j]
            tmp_xmax=b_xmax[j]
            tmp_result=np.stack((tmp_action,tmp_startend,tmp_xmin,tmp_xmax),axis=1)
