...
def TEM_Train(X_feature,Y_feature,LR,istrain,config):
    ...
    anchors_background = net[:,:,2]
    anchors = tf.stack([anchors_action, anchors_startend, anchors_background])
...
class Config(object):
    def __init__(self):
        ...
        self.learning_rates=[0.003]*2000
...
if __name__ == "__main__":
    ...
    Y_background = tf.subtract(one_tensor,Y_startend)
    Y_background = tf.subtract(Y_background,Y_action)
    Y_feature = tf.stack([Y_action, Y_startend, Y_background])
    ...
    for epoch in range(0,config.training_epochs):
        ...
        while True:
            try:
                _,out_loss,print_Y=sess.run([optimizer,loss,out_Y], feed_dict={LR:config.learning_rates[epoch], istrain:True})
                # print('--------------')
                # for j in print_Y:
                #     for k in j:
                #         print(k)
        ...
        train_info_str = "Epoch-%d Train Loss - %.05f" %(epoch,train_info["loss"][-1])
        val_info_str = "Epoch-%d Val   Loss - %.05f" %(epoch,val_info["loss"][-1])
        print(nowTime)
        f.write(nowTime+'\n')
